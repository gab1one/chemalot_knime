...
public class CommandList {
	...
	public static CMDProgramDefinition SDF_CONCATENATER;
	public static CMDProgramDefinition SDF_KNIME_BRIDGE;
	public static CMDProgramDefinition SDF_REMOTE_WRITER;
	public static CommandList DEFAULT;

	public static AtomicBoolean trip = new AtomicBoolean(false);

	public static synchronized void init() { // order dependent!!!
		if (!trip.get()) {
			SDF_CONCATENATER = createSDFConcatenator();
			SDF_KNIME_BRIDGE = createKnimeBridge();
			SDF_REMOTE_WRITER = createRemoteWriter();

			DEFAULT = new CommandList(GNEPlugin.getCmdConfigFilePath());
			trip.compareAndSet(true, true);
		}
	}
	...
	CommandList(final String cmdConfigFilePath) {
		...
		try {
			...
			programMap = new HashMap<>(programs.size() + 10);

			programs.forEach(pd -> programMap.put(pd.getName(), pd));

			// add static nodes to map for de-serialization but not to programs which
	...
	private static CMDProgramDefinition createRemoteReader() {
		...
		return InternalCMDProgramDefinition.createCMDProgramDefinition(name, name, command, subfolder, CMDPortType.NONE,
				CMDPortType.SINGLE_SDF, "", "", defOpts, help);
	}
	...
	private static CMDProgramDefinition createRemoteWriter() {
		...
		return new InternalCMDProgramDefinition(name, name, command, subFolder, CMDPortType.SINGLE_SDF, null, "", "",
				defOpts, help) {

			@Override
			public CommandObject createCommandObject(final CommandObject parentCommand, final String userOptions,
					final String mysubOptions) {
	...
	public List<CMDProgramDefinition> getDefinitions(final CommandType type) {
		ArrayList<CMDProgramDefinition> res = new ArrayList<CMDProgramDefinition>(programs.size());
	...
	/**
	 * Get list of all {@link CMDProgramDefinition}'s for Knime Node with given
	 * Name.
	 */
